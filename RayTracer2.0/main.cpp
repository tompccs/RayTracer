...
void flexirun_empty(double runs, int start, int end, bool matlabprint, bool debug, bool fulldebug, bool wavelengthprint){
...
void flexirun_empty2(double runs, int start, int end, bool matlabprint, bool debug, bool fulldebug, bool wavelengthprint){
    ...
    double r = 4; //radius of curvature
    double l = 10; //length of lsc
    double height = 10; //height of lsc
    double width = 0.4; //width of lsc/thickness
    ...
    Test reader;
    Sheet ss = LSC->GetStartSheet();
    reader.PrintPoint(ss.GetA());
    reader.PrintPoint(ss.GetB());
    reader.PrintPoint(ss.GetC());
    reader.PrintVector(ss.GetNormal());
    reader.PrintPoint(ss.Centre());
    
    Point3D scs = ss.Centre() + ss.GetNormal();
    reader.PrintPoint(scs);
    Vector3D sxs = - ss.GetNormal();
    ...
    for(int wavelength = start; wavelength<= end; wavelength++){
        ...
        for(int i = 0; i<runs; i++){
            ...
            photon->SetPosition(scs);
            photon->SetMomentum(sxs);
            photon->SetWavelength(wavelength);
            photon->SetRandomPolarisation();
...
void flexirun_pof(double runs, int start, int end, bool matlabprint, bool debug, bool fulldebug, bool wavelengthprint){
    ...
    SSA = LSC->GetStartSheet().GetA();
    SSB = LSC->GetStartSheet().GetB();
    SSC = LSC->GetStartSheet().GetC();
    ...
    for(int wavelength = start; wavelength<= end; wavelength++){
        ...
        for(int i = 0; i<runs; i++){
            ...
            Point3D spos = source->GetA() + (calc->Random(1) * VEC_AB) + (calc->Random(1) * VEC_AC) - sdir;
...
void pof(double runs, int start, int end, bool matlabprint, bool debug, bool fulldebug, bool wavelengthprint){
    ...
    //double l = 10; //length of lsc
    double height = 1; //height of lsc
    double width = 1; //width of lsc/thickness
    Point3D centrepoint(-(r),0,0);
    ellipse e(centrepoint,r,r);
    e.SetA(r);
    e.SetB(r);
    arc arcy(e,-M_PI_2,M_PI_2);
    ...
    for(int wavelength = start; wavelength<= end; wavelength++){
        ...
        for(int i = 0; i<runs; i++){
            ...
            photon->SetPosition(Point3D(-r-1,sy,sz));
            photon->SetMomentum(Vector3D(1,0,0));
            photon->SetWavelength(wavelength);
            photon->SetRandomPolarisation();
...
void POF_FLAT_run(double runs,int lscs, int start, int end, bool debug, bool matlabprint, bool wavelengthprint){
    ...
    Point3D LA = lsc->GetLeft().GetA();
    Point3D LB = lsc->GetLeft().GetB();
    Point3D LC = lsc->GetLeft().GetC();

    
    //Point3D G (lsc->GetLeft().GetA());
    //Point3D H (lsc->GetLeft().GetB());
    //Point3D I (lsc->GetLeft().GetC());
    ...
    for(int wavelength = start; wavelength<= end; wavelength++){
        ...
        for(int i = 0; i<runs; i++){
            ...
            sz = source->GetA().z - calc->Random(1) * source->GetABLength();
            sy = source->GetA().y + calc->Random(1) * source->GetACLength();
            
            photon->SetPosition(Point3D(0.9,sy,sz));
            photon->SetMomentum(Vector3D(1,0,0));
            photon->SetWavelength(wavelength);
            photon->SetRandomPolarisation();
            ...
            while(world->ReturnPhotonInside() & photon->PhotonAliveCheck()){ //While photon is inside world and alive.
                if(!objects->PhotonInMaterial()){ //If photon is not in a LSC.
                    ...
                    if(photon->GetAbsorbLength()<=objects->CurrentMaterial()->GetInterfaceDistance(photon)){ //If absorption = next event.
                        ...
                        if((objects->CurrentMaterial()->GetInterfaceSheet(photon) == objects->CurrentMaterial()->GetRight())){
                            photon->PhotonKill(); //If sheet is not top or bottom. Kill photon + add counters.
                            hits++;
                            thishits++;
                            if(debug){
...
void hamid(double runs,int lscs, int start, int end, bool debug, bool matlabprint, bool wavelengthprint){
...
int main(int argc, const char * argv[]){
    ...
    //run(2000,1,350,520,0,0,1);
    
    hamid(2000,1,350,520,0,0,1);

   
    //flexirun(2000, 350, 520, 0, 0, 0, 1);
    
    //pof(20, 350, 520, 0, 1, 1, 1);
    
    //POF_FLAT_run(1000,1,350,520,0,0,1);
    
    //flexirun_pof(300, 450, 450, 0, 1,  1, 1);

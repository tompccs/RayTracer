...
void clearfibre(double runs, int start, int end, bool matlabprint, bool debug, bool fulldebug, bool wavelengthprint){
    ...
    Sheet source = LSC->GetStartSheet();
...
void FLEX(double runs, int start, int end, bool matlabprint, bool debug, bool fulldebug, bool wavelengthprint){
    ...
    LSC->ReadData(1);
    LSC->SetConcentration(1e-4);
    LSC->Set(centrepoint, width, l, height, r, n);
        ...
        for(int i = 0; i<runs; i++){
            ...
            while(world->ReturnPhotonInside() & photon->PhotonAliveCheck()){ //While photon is inside world and alive.
                if(!LSC->GetPhotonInside()){ //If photon is not in a LSC.
                    ...
                    LSC->CalculateDistances(*photon);
                    
                    if(world->GetInterfaceDistance(photon)<LSC->IntersectionDistance()){ //If next boundary is exit.
                        photon->PhotonKill();
                        photon->SetExit();
                        world->SetPhotonInside(0);
                        if(debug) {
                        ...
                        inout->FlexiIn(photon, world, LSC, debug); //Else, entrance reflect/refract event.
                        if(matlabprint && LSC->GetPhotonInside()) photonpath.push_back(photon->GetPosition());
                    }
                }
                    ...
                    if(photon->GetAbsorbLength()<=LSC->IntersectionDistance()){ //If absorption = next event.
                        LSC->AbsorptionEvent(photon,debug,matlabprint,dyeabs,photonpath); //Absorption event.
                    }
                    
                    else{ //If boundary is next event.
                        
                        int nextinterface = LSC->GetNextInterface();
                        ...
                        if((nextinterface!=2 && nextinterface!=3)){
                            ...
                            if(debug){
                                cout<<"Hit on sheet "<<nextinterface<<" at interface point: ";
                                print->PrintPoint(photon->GetPosition()+photon->GetMomentum()*LSC->IntersectionDistance());
                                cout<<endl;
                            }
                        }
                        else{
                            inout->FlexiOut(photon, LSC, world, debug); //Otherwise exit reflect/refract event.
                            if(matlabprint && world->PointinBox(photon)) photonpath.push_back(photon->GetPosition());
